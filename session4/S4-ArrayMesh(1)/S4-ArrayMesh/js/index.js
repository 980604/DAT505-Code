...
console.log("X:" +x+ ",Y : " +y+ ",Z:" +z);
  ...
  if (x >= 0 && y >= 0 && z >= 0){
        ...
        boxMaterial = new THREE.MeshLambertMaterial({color: 0xFF0078});

      } else if ( x >= 0 && y <=0 && z <= 0){
        boxMaterial =new THREE.MeshLambertMaterial({color:0xF65280});
      }else if ( x <= 0 && y <=0 && z >= 0){
        boxMaterial =new THREE.MeshLambertMaterial({color:0xF67280});
      }else if ( x >= 0 && y >=0 && z <= 0){
        boxMaterial =new THREE.MeshLambertMaterial({color:0xF67250});
      }else if ( x <= 0 && y >=0 && z <= 0){
        boxMaterial =new THREE.MeshLambertMaterial({color:0xF67270});
      }else if ( x >= 0 && y <=0 && z <= 0){
        boxMaterial =new THREE.MeshLambertMaterial({color:0xF67670});
      }else{
        boxMaterial =new THREE.MeshLambertMaterial({color:0xF67430});
      }
